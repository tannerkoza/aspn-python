...
class MeasurementTimeFrequencyDifferenceErrorModel(Enum):
    """
...
@dataclass
class MeasurementTimeFrequencyDifference(AspnBase):
    """
    Time and frequency difference between two clocks or timing sources. This measurement requires
    both time and frequency to be provided.
    ...
    covariance - NumpyMatrix[float]:
            Covariance information for the time/frequency measurement. Using zero-indexing, the
            [0,0] element has units of sec^2, the [1,1] element has units of Hertz^2, and the
            off-diagonal terms have units of sec*Hertz.

    error_model - MeasurementTimeFrequencyDifferenceErrorModel:
