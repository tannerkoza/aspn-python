...
class measurement_accumulated_distance_traveled(object):
    """
    Accumulated distance traveled over the time period from (time_of_validity -
    delta_t) to time_of_validity. For example, a sensor that travels (over the
    observation time period) in one direction 1m and reverses course to travel 1m
    back (and ends at its starting position) would have an accumulated distance
    traveled of 2m. Sensor examples may include an odometer or a wheel encoder.
    """

    __slots__ = [
        "icd_measurement_accumulated_distance_traveled",
        "header",
        "time_of_validity",
        "delta_t",
        "obs",
    ...
    def decode(data: bytes):
        ...
        if (
            buf.read(8)
            != measurement_accumulated_distance_traveled._get_packed_fingerprint()
        ):
    ...
    def _decode_one(buf):
        ...
        self.icd_measurement_accumulated_distance_traveled = struct.unpack(
            ">b", buf.read(1)
        )[0]
    ...
    def _get_hash_recursive(parents):
        if measurement_accumulated_distance_traveled in parents:
            return 0
    ...
    def _get_packed_fingerprint():
        if measurement_accumulated_distance_traveled._packed_fingerprint is None:
            measurement_accumulated_distance_traveled._packed_fingerprint = struct.pack(
                ">Q", measurement_accumulated_distance_traveled._get_hash_recursive([])
            )
    ...
    def get_hash(self):
        ...
        return struct.unpack(
            ">Q", measurement_accumulated_distance_traveled._get_packed_fingerprint()
        )[0]
