...
class measurement_velocity(object):
    """
    Velocity is time rate of change of position with respect to the user-specified
    reference frame. May be one, two, or three dimensional.
    """

    __slots__ = [
        "icd_measurement_velocity",
        "header",
        "time_of_validity",
        "reference_frame",
        "x",
    ...
    """ Velocity with respect to earth-centered inertial frame, expressed in an earth-centered inertial frame. See also ECi coordinate frame definition for x, y, and z definitions. """
    REFERENCE_FRAME_ECEF = 1
    """ Velocity with respect to earth-fixed frame expressed in earth-centered, earth-fixed (ECEF) frame. See also ECEF coordinate frame definition for x, y, and z definitions. """
    ...
    """ Velocity with respect to earth-fixed frame expressed in sensor coordinate frame. Sensor coordinate frame is defined in sensor metadataHeader. """
    ERROR_MODEL_NONE = 0
    ...
    def _encode_one(self, buf):
        ...
        buf.write(
            struct.pack(
                ">bdddh", self.reference_frame, self.x, self.y, self.z, self.num_meas
            )
        )
    ...
    def _decode_one(buf):
        ...
        self.reference_frame, self.x, self.y, self.z, self.num_meas = struct.unpack(
            ">bdddh", buf.read(27)
        )
        self.covariance = []
        for i0 in range(self.num_meas):
    ...
    def _get_hash_recursive(parents):
        if measurement_velocity in parents:
            return 0
    ...
    def _get_packed_fingerprint():
        if measurement_velocity._packed_fingerprint is None:
            measurement_velocity._packed_fingerprint = struct.pack(
                ">Q", measurement_velocity._get_hash_recursive([])
            )
