...
class type_navsim_satnav_obs(object):
    ...
    def __init__(self):
        self.icd_type_navsim_satnav_obs = 0
        """
        ...
        """
        Description: String field which describes the Satellite System that generated the observations.
        Units: none
        LCM Type: string
        """

        self.signal_descriptor = ""
        """
        Description: Navsim signal
        Units: none
        LCM Type: string
        """
    ...
    def encode(self):
        buf = BytesIO()
        buf.write(type_navsim_satnav_obs._get_packed_fingerprint())
        self._encode_one(buf)
        return buf.getvalue()

    def _encode_one(self, buf):
        buf.write(struct.pack(">b", self.icd_type_navsim_satnav_obs))
        ...
        buf.write(b"\0")
        __signal_descriptor_encoded = self.signal_descriptor.encode('utf-8')
        buf.write(struct.pack('>I', len(__signal_descriptor_encoded)+1))
        buf.write(__signal_descriptor_encoded)
        buf.write(b"\0")
        __prn_encoded = self.prn.encode('utf-8')
        buf.write(struct.pack('>I', len(__prn_encoded)+1))
        buf.write(__prn_encoded)
        buf.write(b"\0")
        buf.write(struct.pack(">ddfbdfdffibbbbh", self.frequency, self.pseudorange, self.pseudorange_variance, self.pseudorange_rate_type, self.pseudorange_rate, self.pseudorange_rate_variance, self.carrier_phase, self.carrier_phase_variance, self.c_n0, self.lock_count, self.iono_correction_source, self.iono_correction_applied, self.tropo_correction_applied, self.signal_bias_correction_applied, self.num_integrity))
    ...
    def decode(data: bytes):
        if hasattr(data, 'read'):
            ...
            buf = BytesIO(data)
        if buf.read(8) != type_navsim_satnav_obs._get_packed_fingerprint():
            raise ValueError("Decode error")
        return type_navsim_satnav_obs._decode_one(buf)

    @staticmethod
    def _decode_one(buf):
        self = type_navsim_satnav_obs()
        self.icd_type_navsim_satnav_obs = struct.unpack(">b", buf.read(1))[0]
        ...
        __signal_descriptor_len = struct.unpack('>I', buf.read(4))[0]
        self.signal_descriptor = buf.read(__signal_descriptor_len)[:-1].decode('utf-8', 'replace')
        __prn_len = struct.unpack('>I', buf.read(4))[0]
        self.prn = buf.read(__prn_len)[:-1].decode('utf-8', 'replace')
        self.frequency, self.pseudorange, self.pseudorange_variance, self.pseudorange_rate_type, self.pseudorange_rate, self.pseudorange_rate_variance, self.carrier_phase, self.carrier_phase_variance, self.c_n0, self.lock_count, self.iono_correction_source = struct.unpack(">ddfbdfdffib", buf.read(54))
    ...
    def _get_hash_recursive(parents):
        if type_navsim_satnav_obs in parents: return 0
        newparents = parents + [type_navsim_satnav_obs]
        tmphash = (0xd88a76176aaa1e73+ aspn23_lcm.type_integrity._get_hash_recursive(newparents)) & 0xffffffffffffffff
    ...
    def _get_packed_fingerprint():
        if type_navsim_satnav_obs._packed_fingerprint is None:
            type_navsim_satnav_obs._packed_fingerprint = struct.pack(">Q", type_navsim_satnav_obs._get_hash_recursive([]))
        return type_navsim_satnav_obs._packed_fingerprint

    def get_hash(self):
        """Get the LCM hash of the struct"""
        return struct.unpack(">Q", type_navsim_satnav_obs._get_packed_fingerprint())[0]
