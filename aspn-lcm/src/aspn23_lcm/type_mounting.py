...
class type_mounting(object):
    """
    Describes the translational and angular offset between the sensor frame and the
    platform body frame. Platform body frame is defined using axes in forward,
    right, and down convention, and the origin is system defined. This type also
    contains the uncertainty associated with the translational and angular offset.
    """
    ...
    def __init__(self):
        ...
        """
        Description: Four element quaternion, q = [a, b, c, d], where a = cos(phi/2), b = (phi_x/phi)*sin(phi/2), c = (phi_y/phi)*sin(phi/2), and d = (phi_z/phi)*sin(phi/2). In this description, the vector [phi_x, phi_y, phi_z] represents the rotation vector that describes the frame rotation to be applied to the "reference" frame (ECI, ECEF, or NED) to rotate it into the axes that describe the measured attitude, and the value phi is the magnitude of the [phi_x, phi_y, phi_z] vector. Orientation is optional in the sense that orientation shall be specified except in the case that orientation is meaningless, for example, in the case of an RF antenna mounting. See "conventions" documentation for more detailed information
        Units: none
        LCM Type: double[4]
        """

        self.orientation_tilt_error_covariance = [
            [0.0 for dim1 in range(3)] for dim0 in range(3)
        ]
        """
        Description: Tilt error covariance matrix. This matrix represents the uncertainty in the "tilt errors" that represent the additional rotation to be applied to the provided attitude quaternion in order to convert it to the true attitude with no errors. By convention, these "tilt errors" are expressed in the reference frame (ECI, ECEF, or NED). Tilt error covariance is optional only because orientation itself is optional. Orientation error covariance shall be provided if orientation is provided. See "conventions" documentation for more detailed information
        Units: rad^2
        LCM Type: double[3][3]
        """
    ...
    def _encode_one(self, buf):
        ...
        for i0 in range(3):
            buf.write(
                struct.pack(">3d", *self.orientation_tilt_error_covariance[i0][:3])
            )
    ...
    def _decode_one(buf):
        ...
        self.orientation_tilt_error_covariance = []
        for i0 in range(3):
            self.orientation_tilt_error_covariance.append(
                struct.unpack(">3d", buf.read(24))
            )
        return self

    @staticmethod
    def _get_hash_recursive(parents):
        if type_mounting in parents:
            return 0
    ...
    def _get_packed_fingerprint():
        if type_mounting._packed_fingerprint is None:
            type_mounting._packed_fingerprint = struct.pack(
                ">Q", type_mounting._get_hash_recursive([])
            )
