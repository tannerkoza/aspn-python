...
class metadata_GPS_Mnav_ephemeris(object):
    """
    MNAV Ephemeris describing GPS satellite locations. Definitions and usage are
    covered in ICD-GPS-200L, Section 20.3.3.4 and following, with additional
    military use definitions and usage covered in ICD-GPS-700D.
    """

    __slots__ = [
        "icd_metadata_GPS_Mnav_ephemeris",
        "info",
        "time_of_validity",
        "week_number",
        "prn",
    ...
    def _decode_one(buf):
        ...
        (
            ...
            self.isa_l1_m_s,
            self.isa_l2_m_s,
        ) = struct.unpack(">dddddddddddddddd", buf.read(128))
        return self

    @staticmethod
    def _get_hash_recursive(parents):
        if metadata_GPS_Mnav_ephemeris in parents:
            return 0
    ...
    def _get_packed_fingerprint():
        if metadata_GPS_Mnav_ephemeris._packed_fingerprint is None:
            metadata_GPS_Mnav_ephemeris._packed_fingerprint = struct.pack(
                ">Q", metadata_GPS_Mnav_ephemeris._get_hash_recursive([])
            )
    ...
    def get_hash(self):
        ...
        return struct.unpack(
            ">Q", metadata_GPS_Mnav_ephemeris._get_packed_fingerprint()
        )[0]
