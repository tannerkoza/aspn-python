...
class type_satnav_obs(object):
    """
    Defines measurements that come from a satnav receiver tracking a single signal
    ...
    __typenames__ = [
        ...
        "aspn23_lcm.type_satnav_signal_descriptor",
        "int16_t",
        "double",
        "double",
        "float",
    ...
    """ Change in pseudorange (integrated Doppler) over a specified time interval. The interval is provided in metadata_satnav_obs. Units = m Cov. Units = m^2 """
    ...
    def __init__(self):
        ...
        """
        Description: Satellite signal descriptor as defined in RINEX 3.05 Units: none
        LCM Type: aspn23_lcm.type_satnav_signal_descriptor
        """

        self.prn = 0
        """
        Description: PRN code which identifies satellite (or slot number, in the case of GLONASS) for the observables
        Units: none
        LCM Type: int16_t
        """
        ...
        self.frequency = 0.0
        """
        Description: Center frequency of signal. For GLONASS, this should be the center frequency of the frequency-shifted signal with the k value taken into account
        Units: Hz
        LCM Type: double
        """
        ...
        """
        Description: Pseudorange measurement Units: m
        LCM Type: double
        """

        self.pseudorange_variance = 0.0
        """
        Description: Variance of pseudorange measurement noise/multipath. This value must be provided if the pseudorange measurement is provided. Note: This is not intended to represent clock, atmospheric, or satellite position errors
        Units: m^2
        LCM Type: float
        """
        ...
        """
        Description: Variance of pseudorange_rate noise. This value must be provided if the pseudorange_rate measurement is provided
        Units: Hz^2 or m^2
        LCM Type: float
        """
        ...
        """
        Description: Carrier-phase (integrated Doppler) measurement. This measurement has an N cycle ambiguity (where N is unknown and arbitrary integer)
        ...
        """
        Description: Variance of carrier-phase noise/multipath. This value must be provided if the carrier-phase measurement is provided. Note: This is not intended to represent clock, atmospheric, or satellite position errors or the integer N ambiguity
        Units: cycles^2
        LCM Type: float
        """
        ...
        self.c_n0 = 0.0
        """
        Description: Carrier to noise density ratio. C/N0 = 10[log10(S/N0)] Units: db-Hz
        LCM Type: float
        """

        self.lock_count = 0
        """
        Description: Number of observations with continuous tracking (no cycle slips). Value of -1 means that there is no loss of lock information available. First lock_count after cycle slip has occurred will have a value of 0. The lock_count only applies to the carrier-phase measurement
        Units: none
        LCM Type: int32_t
        """
        ...
        """
        Description: Identifies if the ionospheric correction has been applied to the measurement. 1 = Applied
        Units: none
        LCM Type: boolean
        """
        ...
        """
        Description: Identifies if the tropospheric correction has been applied to the measurement. 1 = Applied
        Units: none
        LCM Type: boolean
        """
        ...
        """
        Description: Represents biases within the transmitting signal that are identified in the downlink data, Group Delay for LNAV, and Interchannel Signal Correction (ISC) message for MNAV. This term indicates whether the correction is known, being applied to Pseudorange Correction, and its associated estimated error applied to the ERE. When invalid the correction and estimated error are not known and therefore not applied. 1 = Valid
        Units: none
        LCM Type: boolean
        """
    ...
    def _encode_one(self, buf):
        ...
        assert (
            self.signal_descriptor._get_packed_fingerprint()
            == aspn23_lcm.type_satnav_signal_descriptor._get_packed_fingerprint()
        )
        ...
        buf.write(
            struct.pack(
                ...
                self.pseudorange_rate_type,
                self.pseudorange_rate,
                self.pseudorange_rate_variance,
                self.carrier_phase,
                self.carrier_phase_variance,
                self.c_n0,
                self.lock_count,
                self.iono_correction_source,
                self.iono_correction_applied,
                self.tropo_correction_applied,
                self.signal_bias_correction_applied,
                self.num_integrity,
            )
        )
    ...
    def _decode_one(buf):
        ...
        self.signal_descriptor = aspn23_lcm.type_satnav_signal_descriptor._decode_one(
            buf
        )
        (
            self.prn,
            ...
            self.lock_count,
            self.iono_correction_source,
        ) = struct.unpack(">hddfbdfdffib", buf.read(56))
    ...
    def _get_packed_fingerprint():
        if type_satnav_obs._packed_fingerprint is None:
            type_satnav_obs._packed_fingerprint = struct.pack(
                ">Q", type_satnav_obs._get_hash_recursive([])
            )
