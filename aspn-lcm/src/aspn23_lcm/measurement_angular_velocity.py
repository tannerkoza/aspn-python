...
class measurement_angular_velocity(object):
    """
    Three dimensional angular velocity relative to and expressed in a user-defined
    reference frame.
    """

    __slots__ = [
        "icd_measurement_angular_velocity",
        "header",
        "time_of_validity",
        "reference_frame",
        "imu_type",
    ...
    """ Angular velocity with respect to earth-centered inertial frame, expressed in an earth-centered inertial frame. See also ECi coordinate frame definition for x, y, and z definitions. """
    REFERENCE_FRAME_ECEF = 1
    """ Angular velocity with respect to earth-fixed frame expressed in earth-centered, earth-fixed (ECEF) frame. See also ECEF coordinate frame definition for x, y, and z definitions. """
    REFERENCE_FRAME_NED = 2
    """ Angular velocity with respect to earth-fixed frame expressed in the North-East-Down (NED), local level frame. See also NED coordinate frame definition for N, E, and D definitions. """
    REFERENCE_FRAME_SENSOR = 3
    """ Angular velocity with respect to an inertial frame expressed in sensor coordinate frame. Sensor coordinate frame is defined in sensor metadataHeader. """
    IMU_TYPE_INTEGRATED = 0
    """ delta rotation measurements. Angular rate integrated over last measurement period expressed in radians. Time of validity is the end of the integration period. Integration period is from the previous measurement to the current measurement. """
    IMU_TYPE_SAMPLED = 1
    ...
    def _get_hash_recursive(parents):
        if measurement_angular_velocity in parents:
            return 0
    ...
    def _get_packed_fingerprint():
        if measurement_angular_velocity._packed_fingerprint is None:
            measurement_angular_velocity._packed_fingerprint = struct.pack(
                ">Q", measurement_angular_velocity._get_hash_recursive([])
            )
    ...
    def get_hash(self):
        ...
        return struct.unpack(
            ">Q", measurement_angular_velocity._get_packed_fingerprint()
        )[0]
