...
class measurement_speed(object):
    """
    Speed is the magnitude of the velocity vector with respect to the user-specified
    reference frame.
    """

    __slots__ = [
        "icd_measurement_speed",
        "header",
        "time_of_validity",
        "reference",
        "speed",
    ...
    REFERENCE_TAS = 0
    """ Speed with respect to air measured as true airspeed (TAS) with respect to air mass expressed in sensor coordinate frame. """
    REFERENCE_IAS = 1
    """ Speed with respect to air measured as indicated airspeed (IAS) with respect to air mass expressed in sensor coordinate frame; for example, dynamic pressure from a pitot tube converted to airspeed without compensation for instrument error, position, altitude, or temperature. """
    REFERENCE_WATER = 2
    ...
    def _get_hash_recursive(parents):
        if measurement_speed in parents:
            return 0
    ...
    def _get_packed_fingerprint():
        if measurement_speed._packed_fingerprint is None:
            measurement_speed._packed_fingerprint = struct.pack(
                ">Q", measurement_speed._get_hash_recursive([])
            )
