...
class measurement_altitude(object):
    ...
    REFERENCE_AGL = 0
    """ height above ground level (AGL). Generally, position and terrain elevation data are required to convert between AGL and (MSL or HAE). """
    REFERENCE_MSL = 1
    ...
    ERROR_MODEL_FOGM = 1
    """ First-order Gauss-Markov (FOGM) process, defined by the following parameters (num_error_model_params = 3): 1) standard_deviation, units: m, description: standard deviation of a scalar, first order Gauss-Markov measurement noise model. 2) time_constant, units: seconds, description: time constant of a scalar, first order Gauss-Markov measurement noise model. 3) initial_standard_deviation, units: m, description: initial standard deviation of a scalar, first order Gauss-Markov measurement noise model. """
    ...
    def _get_hash_recursive(parents):
        if measurement_altitude in parents:
            return 0
    ...
    def _get_packed_fingerprint():
        if measurement_altitude._packed_fingerprint is None:
            measurement_altitude._packed_fingerprint = struct.pack(
                ">Q", measurement_altitude._get_hash_recursive([])
            )
