...
class type_direction_3d_to_point(object):
    """
    3D direction to or from a point as defined by the reference enum. Information
    ...
    __slots__ = [
        "icd_type_direction_3d_to_point",
        "remote_point",
        "reference_frame",
        "obs",
        "covariance",
    ...
    """ Azimuth (first component of obs) is the angle between the sensor frame x-axis (boresight) and the projection of the point onto the sensor frame's x-y plane (positive right-hand rule about the sensor frame z-axis). Elevation (second component of obs) is the angle between the sensor frame's x-y plane and the vector from the sensor to the point (positive right-hand rule about the intermediate axis formed by the azimuth rotation). The covariance is for the azimuth and elevation errors. Units for the observation are in rad, covariance rad^2. """
    REFERENCE_FRAME_PIXEL = 1
    """ The pixel coordinates of the point in an image (sub-pixel resolution is allowed). When combined with the camera model, these can be converted to the NORMALIZED_IMAGE reference frame representation. Units for the observation are in pixels, covariance pixels^2. """
    REFERENCE_FRAME_NORMALIZED_IMAGE = 2
    """ 2D position from the sensor frame origin to the point expressed in components y and z of the sensor frame normalized by the boresight distance. The x component of the sensor frame is the boresight axis. Units for the observation are m/m, covariance (m/m)^2. """
    REFERENCE_FRAME_SINE_SPACE = 3
    """ 2D position from the sensor frame origin to the point expressed in components y and z of the sensor frame normalized by the range to the point. The x component of the sensor frame is the boresight axis. Units for the observation are m/m, covariance (m/m)^2. """
    ERROR_MODEL_NONE = 0
    ...
    def _get_hash_recursive(parents):
        if type_direction_3d_to_point in parents:
            return 0
    ...
    def _get_packed_fingerprint():
        if type_direction_3d_to_point._packed_fingerprint is None:
            type_direction_3d_to_point._packed_fingerprint = struct.pack(
                ">Q", type_direction_3d_to_point._get_hash_recursive([])
            )
    ...
    def get_hash(self):
        ...
        return struct.unpack(
            ">Q", type_direction_3d_to_point._get_packed_fingerprint()
        )[0]
