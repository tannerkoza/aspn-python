...
class measurement_time_difference(object):
    ...
    __slots__ = [
        "icd_measurement_time_difference",
        "header",
        "time_of_validity",
    ...
    def __init__(self):
        ...
        """
        Description: Whole number nanoseconds in time difference (time from clock_id1) - (time from clock_id2). This value shall be positive when (time from clock_id1) - (time from clock_id2) is greater than or equal to zero. This value shall be negative when (time from clock_id1) - (time from clock_id2) is less than zero.Additionally, the timestamp's digits of precision are included as `digits_of_precision`
        Units: nanoseconds
        LCM Type: int64_t
        """
        ...
        """
        Description: Whole number attoseconds in time difference (time from clock_id1) - (time from clock_id2). Must be less than 1e9. This value shall be positive when (time from clock_id1) - (time from clock_id2) is greater than or equal to zero. This value shall be negative when (time from clock_id1) - (time from clock_id2) is less than zero. This is only needed if the time of validity is known to more precision than 1ns; otherwise, set time_of_validity_attosec = 0. Be sure to report the timestamp's digits of precision. The time difference in seconds is calculated as time difference = time_diff_nsec * 1e-9 + time_diff_attosec * 1e-18 It shall be assumed that time_diff_nsec and time_diff_attosec have the same sign. Therefore, the time difference is positive when (time from clock_id1) - (time from clock_id2) is positive and the time difference is negative when (time from clock_id1) - (time from clock_id2) is negative. A cautionary note: Both timediff_nsec and time_diff_attosec are signed
        integers and may be positive or negative. It is mandated to make timediff_nsec and time_diff_attosec the same sign; however, care should be taken when constructing the observation components to be sure the summation produces the desired result
        Units: attoseconds
        LCM Type: int32_t
        """
    ...
    def _decode_one(buf):
        ...
        (
            ...
            self.error_model,
            self.num_error_model_params,
        ) = struct.unpack(">ihhqihdbi", buf.read(35))
    ...
    def _get_hash_recursive(parents):
        if measurement_time_difference in parents:
            return 0
    ...
    def _get_packed_fingerprint():
        if measurement_time_difference._packed_fingerprint is None:
            measurement_time_difference._packed_fingerprint = struct.pack(
                ">Q", measurement_time_difference._get_hash_recursive([])
            )
    ...
    def get_hash(self):
        ...
        return struct.unpack(
            ">Q", measurement_time_difference._get_packed_fingerprint()
        )[0]
