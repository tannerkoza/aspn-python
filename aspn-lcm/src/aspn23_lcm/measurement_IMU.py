...
class measurement_IMU(object):
    """
    Inertial Measurement Unit (IMU) measurements from the device's three axis
    accelerometers and three axis gyroscopes.
    """
    ...
    """ delta velocity and delta rotation measurements from the device's three axis accelerometers and three axis gyroscopes. Specific force integrated over last measurement period expressed as delta_v array in m/s. Angular rate integrated over last measurement period expressed as delta_theta array in radians. Time of validity is the end of integration period. Integration period is from previous measurement to the current measurement. """
    IMU_TYPE_SAMPLED = 1
    """ sampled specific force and rotation rates from the device's three axis accelerometers and three axis gyroscopes. Specific force is sampled expressed as an array in m/s/s. Angular rate is sampled and expressed as an array in rad/s. """
    ...
    def _get_packed_fingerprint():
        if measurement_IMU._packed_fingerprint is None:
            measurement_IMU._packed_fingerprint = struct.pack(
                ">Q", measurement_IMU._get_hash_recursive([])
            )
